...
const configuration = new Configuration({
    apiKey: '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',
});
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if (type === "notify" && !messages[0].key.fromMe) {
            try {
                ...
                const isMessageMentionBot = incomingMessages.includes("@6281262163214");
                ...
                if (!isMessageFromGroup) {
                    ...
                    if (incomingMessages.includes('siapa') && incomingMessages.includes('kamu')) {
                        ...
                        async function main() {
                            const hasil = await generateResponse(incomingMessages);
                            console.log(hasil);
                            await sock.sendMessage(
                                senderNumber,
                                { text: hasil},
